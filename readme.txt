=== Plugin Name ===
Plugin URI: http://emspace.gr/
Description: Woocommerce Pay in Store Gateway
Version: 1.0.0
Author: Emspace
Author URI: http://emspace.gr/
Contributors: mpbm23
Tags: ecommerce, e-commerce,  wordpress ecommerce, gateway

Requires at least: 3.8
Tested up to: 4.1
Stable tag: 2.3.3

License: GPLv3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html

Provides a Pay in Store upon pick up Payment Gateway for Woocommerce.

== Description ==

Provides a Pay in Store upon pick up Payment Gateway for Woocommerce. Woocommerce from Woothemes Cash on Delivery (COD) copy to provide Pay in Store as a payment method.


== Frequently Asked Questions ==


Does is work?

Yes, it's just a copy and rename of Cash on Delivery Payment Gateway



== Changelog ==


=Version: 1.0.0=
Initial Release